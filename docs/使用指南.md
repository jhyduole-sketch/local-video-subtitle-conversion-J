...
[返回中文首页](../README.md) · [English](../README.en.md) · [查看更新记录](更新记录.md)

`local-video-subtitle-conversion-J` 是一个本地视频字幕处理工具：输入本地视频或公开视频网址，自动读取内置字幕或识别语音，生成目标语言 `.srt`，并可输出软字幕或固定位置硬字幕 MP4。

本指南包含安装、配置、Web 界面、CLI、模型、输出、缓存、字幕校对和故障排查。想先快速了解项目，请从[中文首页](../README.md)开始。

## 功能要点

- 支持本地视频路径、网页上传视频、YouTube URL、Bilibili URL，以及其他公开单视频网址的通用尝试下载。
- YouTube 地址兼容普通 `watch?v=`、`youtu.be`、Shorts、Live、Embed、旧式 `/v/`，以及 Music YouTube 和无 Cookie 嵌入地址。
- 已适配网站优先使用专用解析；其他 HTTP/HTTPS 地址使用 `yt-dlp` 匿名探测，成功后自动进入字幕流程。
- YouTube 默认优先下载高清视频，不再优先使用低清 360p 格式。
- 字幕来源支持自动判断：优先读视频内置字幕，没有内置字幕时抽音频语音识别；转写为空或高度重复时再尝试画面 OCR。
- macOS 支持系统 Vision 画面 OCR，可从没有对白、只有画面硬字幕的视频生成源 SRT；OCR 引擎接口可以替换为其它本地或云端实现。
...
- 下载 YouTube、Bilibili 或其他受支持的公开视频后再生成字幕。
...
硬字幕是画面像素的一部分，播放器不能关闭，也不能像字幕轨一样直接提取。选择“稳定硬字幕”后，工具会使用 ASS 样式和 FFmpeg 把新字幕烧录进画面，因此需要重新编码视频。当前 macOS 版本可使用 Vision OCR 尝试把画面文字重新识别成可编辑字幕。
...
- 本地翻译模型缓存，使用本地翻译时需要
- macOS Command Line Tools，首次使用系统 Vision OCR 时用于编译轻量辅助程序
...
- 任务运行时可点击“停止任务”。
- 查看历史任务，并继续失败、取消或因服务重启而中断的任务；“清空历史”只删除已结束记录，排队和运行中的任务会保留。
- 查看缓存占用，按类别清理缓存，或使用“清空全部”一次删除全部缓存。
- “清空历史”、分类清理和“清空全部缓存”执行前都会显示确认框，明确说明会删除和会保留的内容。
...
视频网址解析、视频下载、本地 Whisper 和硬字幕烧录都有超时保护。运行时间较长时，任务日志会定时显示“仍在处理”和已用时间；硬字幕烧录若连续 5 分钟没有任何 FFmpeg 进度，会自动终止并给出中文处理建议，避免页面长期停在同一步骤。
...
因此，VAD 适合加速有明显静音区间的视频，但不会成为生成字幕的单点故障。任务日志会记录实际使用的模式和每次降级原因。

### 画面字幕 OCR

OCR 适合“没有有效对白，但画面上已经烧录字幕”的视频。自动模式的顺序是：
...
macOS 首次运行会在 `.subtitle-tool-cache/tools/` 编译并缓存一个轻量 Vision 辅助程序，之后直接复用。视频帧、识别结果和 OCR 源字幕仍按现有缓存规则管理；环境区会显示“画面字幕 OCR”是否可用。
...
OCR 会按固定间隔抽帧，过滤低置信度、边缘台标和纯数字叠层，再把连续帧中的相似文字合并成 SRT 时间轴。花体字、移动文字、低清画面或同屏多处文字仍可能产生漏字和错字，任务完成后建议使用“预览并校对字幕”。

当前内置引擎为 macOS Vision。Linux 服务器启动不会因此失败，但会显示 OCR 引擎不可用；部署到服务器时可以沿统一接口接入 PaddleOCR、Tesseract 或云端 OCR。

任务记录保存在项目根目录的 `.subtitle-tool-state/jobs.sqlite3`。继续任务会创建一个新的关联任务，并复用已完成的下载、音频、转写及翻译批次，不会覆盖原任务记录。`output/` 只保存下载视频、字幕和成品视频。

任务历史和缓存区域的“刷新”只会重新读取当前状态和占用空间，不会删除文件。“清空历史”删除已完成、失败、停止和中断的任务记录，但不会删除 `output/` 成品、缓存或活动任务。
...
```bash
env PYTHONPATH=src python3 -m subtitle_tool.cli 'https://example.com/public-video' \
  --source-lang ja \
  --target-lang zh-CN \
  --transcriber local-whisper \
  --translator z-ai \
...
输入视频路径或 URL。已适配网站优先使用专用逻辑；其他 HTTP/HTTPS 地址会尝试由 `yt-dlp` 匿名解析和下载。
...
`local-nllb-quality` 是当前 Web 使用的 NLLB 1.3B 参数。`local-nllb` 仅为兼容旧命令和历史任务而保留，实际也使用 NLLB 1.3B。
...
一般不需要修改。需要处理超长视频或排查网络问题时，可以在 `.env` 中配置以下秒数：

```dotenv
SUBTITLE_TOOL_DOWNLOAD_PROBE_TIMEOUT_SECONDS=120
SUBTITLE_TOOL_DOWNLOAD_TIMEOUT_SECONDS=7200
SUBTITLE_TOOL_WHISPER_TIMEOUT_SECONDS=7200
SUBTITLE_TOOL_BURN_TIMEOUT_SECONDS=43200
SUBTITLE_TOOL_BURN_INACTIVITY_TIMEOUT_SECONDS=300
```

- `DOWNLOAD_PROBE`：通用公开视频网址解析的最长等待时间。
- `DOWNLOAD`：单个视频下载的最长等待时间。
- `WHISPER`：一次本地语音转写的最长等待时间。
- `BURN`：一次固定位置硬字幕烧录的最长总时间。
- `BURN_INACTIVITY`：硬字幕烧录连续没有进度的最长时间，默认 5 分钟。

超时后任务会停止对应子进程，并显示中文建议和保留的技术信息。通常先检查网络、视频是否可公开访问、磁盘空间和模型路径，再决定是否增大超时。
...
字幕翻译更重视准确性，因此当前版本不再提供600M入口，z.ai 失败后的本地多语言回退也使用1.3B。NLLB 模型较大，首次下载会比较慢，CPU 翻译速度也比本地快速模型慢。工具不会在任务中静默下载模型；Web 页面右侧“环境”区会显示1.3B是否已安装，并给出下载命令。
...
已经下载到 Hugging Face 缓存中的600M模型不会被程序自动删除，需要释放空间时可自行清理对应模型缓存。
...
批量大小会根据模型、运行设备和系统内存自动选择。Apple Metal 可用时会使用适合 GPU 的批量；CPU 模式会在内存较小时主动降低批量，避免一次处理过多字幕导致进程退出。如果仍遇到内存不足，当前批量会自动减半并重试，不需要重新启动整个任务。

需要手动限制批量时，可以在启动服务前设置：

```bash
export LOCAL_TRANSLATION_BATCH_SIZE=2
env PYTHONPATH=src python3 -m subtitle_tool.web --host 127.0.0.1 --port 7860
```

数值越大通常吞吐量越高，但占用内存也越多。稳定优先时建议从 `2` 或 `4` 开始；不设置时交给程序自动判断。

每完成一个 NLLB 批次，工具都会把当前结果写入翻译断点缓存，并在日志中显示已完成条数、已用时间和预计剩余时间。任务失败、停止或服务重启后，通过“继续”重新运行时会跳过已经完成的字幕。旧参数 `local-nllb` 与当前 `local-nllb-quality` 共用同一份 1.3B 翻译缓存。
...
如果 z.ai 在某个目标语言上最终超时或失败，当前任务会打开 z.ai 熔断：后续目标语言不再重复等待 z.ai，而是直接进入本地模型和 OpenAI 回退链路。熔断只在本次任务内有效，新任务仍会重新尝试 z.ai。日志会明确显示“z.ai 已熔断”，便于确认实际使用的引擎。

翻译批次完成后，页面会显示类似 `翻译进度: de · 240/743`，并把批次完成比例映射到任务总进度。模型自己的批次、已用时间和预计剩余时间日志仍会保留。

单次任务中完全相同的源字幕只送入模型一次。例如时间轴上出现十次“谢谢”，工具会翻译一次，再把结果恢复到原来的十个字幕索引和时间段。该优化不会合并输出时间轴，也不会改变字幕出现时间；日志会显示合并数量和复用条数。
...
按类别清理或“清空全部缓存”不会删除已经生成的时间戳任务目录，也不会删除任务历史。清空后，下次处理相同视频时可能需要重新下载、抽取音频、转写、翻译或分析字幕位置。z.ai 和 NLLB 分批翻译中断后，未清理缓存时已成功的批次会保留；继续任务时只处理尚未完成的字幕片段。
...
会尝试读取。画面里的字幕通常是硬字幕，已经成为视频像素，不能像内置字幕轨一样直接导出。macOS 版本可使用系统 Vision OCR 识别画面文字：自动模式会在音频转写为空或高度重复时切换 OCR，也可以直接把“字幕来源”选为“画面字幕 OCR”。识别完成后会生成可编辑的源 SRT，建议在字幕校对区检查错字和时间轴。
...
- 画面字幕 OCR 当前内置实现为 macOS Vision；Linux / Windows 需要接入其它兼容引擎。
- OCR 对花体字、快速动画、低清画面、台标和同屏多处文字可能识别不完整，生成后建议人工校对。
- 自动检测置信度不足时会保守假设原字幕位于底部；可在页面手动覆盖新字幕位置。
- “画面字幕位置检测”和“画面字幕 OCR”是两项功能：前者只判断新字幕放在哪里，后者负责识别画面文字。
